...
const AddUserInformationInitForm = ({
  ...
  return (
    ...
    >
      <section
        className={
          "flex max-h-[calc(100vh-1rem)] w-full flex-col items-center overflow-y-scroll rounded-[1rem] rounded-b-[1rem] bg-white px-[3.5rem] pb-[2rem] pt-[.5rem] outline outline-[0.0625rem] outline-offset-[-0.0625rem] outline-gray2 scrollbar-hide"
        }
      >
        <div className="flex w-full py-4">
          <Link
            href={"/"}
            className="bg-gray relative z-50 scale-100 transform rounded-[1rem] bg-[#F7F7F7] p-2 transition-transform duration-300"
          >
            {/* <MdClose
        ...
        <button
          ...
          className={`mt-5 min-h-[4rem] w-full rounded-[1.5rem] ${formContext.formState.isValid && formContext.getValues("isCheckTerm") && formContext.getValues("isCheckPrivacy") ? "bg-main text-white" : "bg-gray3"}`}
          disabled={
        ...
        <button
          className={`mt-5 min-h-[4rem] w-full rounded-[1.5rem] ${formContext.getValues("isCheckTerm") && formContext.getValues("isCheckPrivacy") ? "bg-main text-white" : "bg-gray3"}`}
          disabled={
            !formContext.getValues("isCheckTerm") ||
            !formContext.getValues("isCheckPrivacy")
          }
