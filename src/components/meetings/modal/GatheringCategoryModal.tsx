...
const GatheringCategoryModal = (props: IGatheringCategoryModalProps) => {
  ...
  return (
    <div
      className={
        "relative w-[calc(100vw-1rem)] max-w-[40rem] rounded-2xl bg-white px-[1rem] py-[2.875rem] md:p-[2.875rem] overflow-scroll"
      }
    >
      <button
      ...
      <div className={"flex flex-col gap-y-[1rem] pt-[2rem]"}>
        ...
        <div className={"flex flex-wrap gap-x-[1rem] gap-y-[.5rem]"}>
          {Object.entries(CATEGORY_MODAL_CATEGORY_LIST).map((i) => (
            <button
              ...
              onClick={() => {
                setMainCategory(i[0]);
                setSubCategory(Object.entries(CATEGORY_MODAL_CATEGORY_LIST[i[0]].subCategory)[0][0]);
              }}
        ...
        <div className={"flex flex-wrap gap-x-[1rem] gap-y-[.5rem] min-h-[2.5rem]"}>
          {
          mainCategory &&
            Object.entries(CATEGORY_MODAL_CATEGORY_LIST[mainCategory].subCategory).map((i) => (
            <button
              key={i[0]}
